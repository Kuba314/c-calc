...
static int call_function(const struct func_def_t *func, const token_list_t *args, uint8_t nargs, enum token_type_t ttype, token_t *result) {
    ...
    // if no args and d0 is not set, change type to complex, because otherwise
    // there'd be no way to call complex functions with no arguments
    if(nargs == 0 && func->d0 == NULL)
        ttype = TT_COMPLEX;
    if(ttype == TT_REAL) {
        if(nargs == 0) {

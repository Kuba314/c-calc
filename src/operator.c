...
complex_t mod_complex(complex_t a, complex_t b) {
    complex_t x = a / b;
    x = floorl(creall(x)) + floorl(cimagl(x)) * I;
    return a - x * b;
}
complex_t fact_complex(complex_t a) {
...
static operator_t OPERATORS[] = {
    ...
    {.sym='%', .type=OP_BINARY, .priority=1, .fc.c2=mod_complex},
    {.sym='^', .type=OP_BINARY, .priority=3, .fc.c2=cpowl},
    {.sym='!', .type=OP_RUNARY, .priority=5, .fc.c1=fact_complex},
